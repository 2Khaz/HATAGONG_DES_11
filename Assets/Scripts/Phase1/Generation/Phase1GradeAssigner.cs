...
namespace HATAGONG.Phase1
{
    public sealed class Phase1GradeAssigner
    {
        ...
        private Phase1TileGradeDefinition Fallback(List<Phase1TileGradeDefinition> list)=>list.FirstOrDefault(x=>x.Grade==config.DefaultFallbackGrade)??list.OrderBy(x=>Math.Abs(x.HpModifier)).ThenBy(x=>x.HpModifier).FirstOrDefault();
