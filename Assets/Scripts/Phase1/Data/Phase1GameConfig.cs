...
namespace HATAGONG.Phase1
{
    ...
    public sealed class Phase1GameConfig : ScriptableObject
    {
        ...
        public void EnsureDefaults()
        {
            ...
            if(gradeDefinitions.Count!=4)gradeDefinitions=new List<Phase1TileGradeDefinition>{new(Phase1TileGrade.Beige,"BEIGE","Beige",0,20,10,5,1,1,1,0,0,0),new(Phase1TileGrade.Brown,"BROWN","Brown",2,30,25,15,1,1,1,0,0,0),new(Phase1TileGrade.Gray,"GRAY","Gray",4,25,30,35,1,1,1,0,0,0),new(Phase1TileGrade.Marble,"MARBLE","Marble",7,25,35,45,1,2,3,2,3,5)};
            gradeDefinitions.Find(x=>x.Grade==Phase1TileGrade.Beige)?.EnsureMinimumCounts(1,1,1);
            gradeDefinitions.Find(x=>x.Grade==Phase1TileGrade.Brown)?.EnsureMinimumCounts(1,1,1);
            gradeDefinitions.Find(x=>x.Grade==Phase1TileGrade.Gray)?.EnsureMinimumCounts(1,1,1);
            gradeDefinitions.Find(x=>x.Grade==Phase1TileGrade.Marble)?.EnsureMinimumCounts(1,2,3);
        ...
        {
            EnsureDefaults();bool ok=true;var enabled=gradeDefinitions.FindAll(x=>x.Enabled);ok&=enabled.Count>0;ok&=gradeDefinitions.Select(x=>x.GradeId).Distinct().Count()==gradeDefinitions.Count;ok&=gradeDefinitions.All(x=>x.HpModifier>=0&&x.HpModifier<=7&&x.Weight(Phase1Difficulty.Easy)>0&&x.Weight(Phase1Difficulty.Normal)>0&&x.Weight(Phase1Difficulty.Hard)>0&&x.MinCount(Phase1Difficulty.Easy)>=0&&x.MinCount(Phase1Difficulty.Normal)>=0&&x.MinCount(Phase1Difficulty.Hard)>=0&&x.MaxCount(Phase1Difficulty.Easy)>=0&&x.MaxCount(Phase1Difficulty.Normal)>=0&&x.MaxCount(Phase1Difficulty.Hard)>=0);ok&=enabled.Any(x=>x.Grade==defaultFallbackGrade);ok&=minimumFinalTileHp>=2&&gradeAssignmentAttempts>0;ok&=easyModifierMin==0&&easyModifierMax==30&&normalModifierMin==0&&normalModifierMax==41&&hardModifierMin==0&&hardModifierMax==55;ok&=gradeVisualSets.Select(x=>x.Grade).Distinct().Count()==gradeVisualSets.Count;
            foreach(Phase1Difficulty d in System.Enum.GetValues(typeof(Phase1Difficulty))){ok&=enabled.All(x=>x.MaxCount(d)==0||x.MinCount(d)<=x.MaxCount(d));ok&=bags.Where(x=>x.Difficulty==d).All(x=>enabled.Sum(g=>g.MinCount(d))<=x.ExpectedTileCount);}
            ...
            ok&=GradeMatches(Phase1TileGrade.Beige,0,20,10,5,1,1,1,0,0,0)&&GradeMatches(Phase1TileGrade.Brown,2,30,25,15,1,1,1,0,0,0)&&GradeMatches(Phase1TileGrade.Gray,4,25,30,35,1,1,1,0,0,0)&&GradeMatches(Phase1TileGrade.Marble,7,25,35,45,1,2,3,2,3,5);
        ...
        private bool GradeMatches(Phase1TileGrade grade,int modifier,int ew,int nw,int hw,int emin,int nmin,int hmin,int emax,int nmax,int hmax){var value=gradeDefinitions.Find(x=>x.Grade==grade);return value!=null&&value.HpModifier==modifier&&value.Weight(Phase1Difficulty.Easy)==ew&&value.Weight(Phase1Difficulty.Normal)==nw&&value.Weight(Phase1Difficulty.Hard)==hw&&value.MinCount(Phase1Difficulty.Easy)==emin&&value.MinCount(Phase1Difficulty.Normal)==nmin&&value.MinCount(Phase1Difficulty.Hard)==hmin&&value.MaxCount(Phase1Difficulty.Easy)==emax&&value.MaxCount(Phase1Difficulty.Normal)==nmax&&value.MaxCount(Phase1Difficulty.Hard)==hmax;}
