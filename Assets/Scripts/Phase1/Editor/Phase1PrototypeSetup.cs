    ...
    public static class Phase1PrototypeSetup
    {
        ...
        public static void ValidateGradeDistribution()
        {
            ...
            foreach(Phase1Difficulty difficulty in System.Enum.GetValues(typeof(Phase1Difficulty)))
            {
                ...
                for(int i=0;i<1000;i++)
                {
                    ...
                    foreach(Phase1TilePlacement tile in board.Tiles){if(string.IsNullOrEmpty(tile.GradeId))nullGrades++;if(tile.BaseHp!=config.GetHp(difficulty,tile.Shape))baseMismatch++;if(tile.MaxHp!=tile.BaseHp+tile.GradeHpModifier+tile.BalanceHpModifier||tile.MaxHp<8||tile.MaxHp>43)hpRange++;}
        ...
        private static void ValidateGenerationMatrix(int seedsPerBag,string label)
        {
            ...
            foreach(var bag in config.Bags)for(int i=0;i<seedsPerBag;i++)
            {
                ...
                if(board!=null){testedTiles+=board.Tiles.Count;minimumHpViolations+=board.Tiles.Count(x=>!x.MinimumHpValid||x.MaxHp<config.MinimumFinalTileHp);hpMismatches+=board.Tiles.Count(x=>x.MaxHp!=x.BaseHp+x.GradeHpModifier+x.BalanceHpModifier);}
                if(board!=null&&board.Tiles.All(x=>x.MinimumHpValid&&x.MaxHp>=config.MinimumFinalTileHp&&x.MaxHp==x.BaseHp+x.GradeHpModifier+x.BalanceHpModifier)){success++;if(seedsPerBag<=10)Debug.Log($"[Phase1][{label}] {bag.Id} seed={usedSeed} tiles={board.Tiles.Count} area={board.Tiles.Sum(x=>x.GridWidth*x.GridHeight)} baseHp={board.Tiles.Sum(x=>x.BaseHp)} gradeModifier={board.Tiles.Sum(x=>x.GradeHpModifier)} balanceModifier={board.Tiles.Sum(x=>x.BalanceHpModifier)} finalHp={board.Tiles.Sum(x=>x.MaxHp)} layout={board.LayoutHash} variant={board.VariantHash}");}
